...
import { FilePlus, Plus, Search, FileDown, FileUp, Receipt } from "lucide-react";
      ...
      cell: ({ row }) => {
        return (
          <div className="flex gap-2">
            <Button
              variant="ghost"
              onClick={() => navigate(`/dashboard/service-orders/${row.original.id}`)}
            >
              Detalhes
            </Button>
            <Button
              variant="ghost"
              onClick={() => navigate(`/dashboard/nfse/from-service-order/${row.original.id}`)}
              title="Gerar NFS-e"
